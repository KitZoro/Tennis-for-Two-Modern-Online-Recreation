    ...
    while True:
                ...
                for key, _, mode in options:
                    if event.unicode == key:
                        if mode == "audio": # added
                            ...
                            ui.music_menu(screen, fonts)
                        else: # added
                            return mode

        screen.fill(BG)
        ui.draw_grid(screen)
        ui.draw_text(screen, large, "TENNIS FOR TWO", (WIDTH // 2, 48), GREEN, center=True)
        ui.draw_text(screen, small, "1958 GAMEPLAY — MODERN FEATURES — EDWIN SKYCROSS STORY", (WIDTH // 2, 78), MUTED, center=True)

        y = 112
        for key, label, _ in options:
            pygame.draw.rect(screen, DARK_PANEL, (210, y - 17, 580, 34), border_radius=9)
        ...
        if ui.SOUND is not None:
            ...
            ui.draw_text(
                screen,
                small,
                f"MUSIC {music_state} {round(ui.SOUND.music_volume * 100)}%: {ui.SOUND.music_name}    "
                f"SFX {sfx_state} {round(ui.SOUND.sfx_volume * 100)}%",
                (WIDTH // 2, 505),
                MUTED,
                center=True,
            )

        ui.draw_text(screen, small, "F6/F7 MUSIC -/+   F8/F9 SFX -/+   M/N MUTE   F11 FULLSCREEN", (WIDTH // 2, 545), MUTED, center=True)
        ui.present(screen)
        ...
        for event in ui.game_events():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if ui.SOUND is not None:
                        if selected == 0:
                            ui.SOUND.set_sfx_volume(ui.SOUND.sfx_volume - 0.10)
                        else:
                            ui.SOUND.set_music_volume(ui.SOUND.music_volume - 0.10)
                ...
                elif ui.handle_audio_hotkey(event):
                    continue
