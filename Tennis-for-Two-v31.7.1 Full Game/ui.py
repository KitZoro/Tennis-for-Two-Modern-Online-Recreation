...
from audio import BUILT_IN_MUSIC, SoundManager, VOLUME_STEP
from config import *
from cpu import CpuController
from test_cpu import DeterministicTestCpu
from game import (
...
AUDIO_STATUS_TEXT = ""
AUDIO_STATUS_UNTIL = 0.0
AUDIO_STATUS_FONT = None
...
def present(logical_screen: pygame.Surface) -> None:
    ...
    if SOUND is not None:
        SOUND.maintain_output_levels()
    _draw_audio_status(logical_screen)
...
def game_events():
    ...
    for event in pygame.event.get():
        ...
        # Volume controls are consumed here so they work in every live screen,
        # including Story Mode scenes and active local/online matches.
        if (
            event.type == pygame.KEYDOWN
            and event.key in (pygame.K_F6, pygame.K_F7, pygame.K_F8, pygame.K_F9)
            and handle_audio_hotkey(event)
        ):
            continue
...
def handle_audio_hotkey(event) -> bool:
    ...
    if event.key == pygame.K_m:
        muted = SOUND.toggle_music()
        _show_audio_status(f"MUSIC {'MUTED' if muted else f'{round(SOUND.music_volume * 100)}%'}")
        return True
    if event.key == pygame.K_n:
        muted = SOUND.toggle_sfx()
        _show_audio_status(f"SFX {'MUTED' if muted else f'{round(SOUND.sfx_volume * 100)}%'}")
        return True
    if event.key == pygame.K_F5:
        SOUND.reload_music()
        _show_audio_status(f"MUSIC RELOADED: {_music_display_name(SOUND.music_name)}")
        return True
    if event.key == pygame.K_F6:
        volume = SOUND.change_music_volume(-1)
        _show_audio_status(f"MUSIC {round(volume * 100)}%")
        return True
    ...
    if event.key == pygame.K_F8:
        volume = SOUND.change_sfx_volume(-1)
        _show_audio_status(f"SFX {round(volume * 100)}%")
        return True
...
def _show_audio_status(message: str) -> None:
    """Show a short confirmation without pausing the current game screen."""
    global AUDIO_STATUS_TEXT, AUDIO_STATUS_UNTIL
    AUDIO_STATUS_TEXT = message
    AUDIO_STATUS_UNTIL = time.monotonic() + 1.6


def _draw_audio_status(screen: pygame.Surface) -> None:
    global AUDIO_STATUS_FONT
    if not AUDIO_STATUS_TEXT or time.monotonic() >= AUDIO_STATUS_UNTIL:
        return
    if AUDIO_STATUS_FONT is None:
        AUDIO_STATUS_FONT = pygame.font.SysFont("consolas", 18, bold=True)

    label = AUDIO_STATUS_FONT.render(AUDIO_STATUS_TEXT, True, WHITE)
    box = label.get_rect()
    box.width += 28
    box.height += 16
    box.topright = (WIDTH - 14, 14)
    pygame.draw.rect(screen, (8, 24, 26), box, border_radius=8)
    pygame.draw.rect(screen, GREEN, box, width=2, border_radius=8)
    screen.blit(label, (box.x + 14, box.y + 8))
...
def menu(screen, fonts) -> Optional[str]:
    ...
    while True:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                for key, _, mode in options:
                    if event.unicode == key:
                        ...
                        # added, it's a bit rough i think // USELESS BECAUSE THIS ISN'T THE REAL MENU
                        # THE REAL MENU IS IN STORY.PY
                        if mode == "audio":
                            audio_menu(screen, fonts)
                        elif mode == "music":
                            music_menu(screen, fonts)
                        else:
        ...
        screen.fill(BG)
        draw_grid(screen)
        draw_text(screen, large, "TENNIS FOR TWO", (WIDTH // 2, 48), GREEN, center=True)
        draw_text(screen, small, "CPU + TLS 1.3 + ROLLBACK + AUTO-PAIR + CUSTOM MUSIC", (WIDTH // 2, 78), MUTED, center=True)
        y = 112
        for key, label, _ in options:
            pygame.draw.rect(screen, DARK_PANEL, (230, y - 17, 540, 34), border_radius=9)
        ...
        if SOUND is not None:
            ...
            draw_text(
                screen, small,
                f"MUSIC {music_state} {round(SOUND.music_volume * 100)}%: {SOUND.music_name}    "
                f"SFX {sfx_state} {round(SOUND.sfx_volume * 100)}%",
                (WIDTH // 2, 505), MUTED, center=True
            )
        draw_text(screen, small, "F6/F7 MUSIC -/+   F8/F9 SFX -/+   M/N MUTE   F11 FULLSCREEN", (WIDTH // 2, 545), MUTED, center=True)
        present(screen)


 # added the next def for the sound menu
def audio_menu(screen, fonts) -> None:
    ...
    while True:
        ...
        for event in game_events():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if SOUND is not None:
                        if selected == 0:
                            SOUND.set_sfx_volume(SOUND.sfx_volume - VOLUME_STEP)
                        else:
                            SOUND.set_music_volume(SOUND.music_volume - VOLUME_STEP)
        ...
        draw_text(
            screen,
            small,
            "UP/DOWN SELECT   LEFT/RIGHT CHANGE 10%   RANGE 0%-200%   ESC BACK",
            (WIDTH // 2, 520),
            MUTED,
            center=True,
        )
...
def _music_display_name(choice: str) -> str:
    """Turn a music filename into a compact, menu-friendly title."""
    if choice == BUILT_IN_MUSIC:
        return choice

    title = choice.rsplit(".", 1)[0]
    if len(title) > 3 and title[:2].isdigit() and title[2] in (" ", "-", "_"):
        title = title[3:]
    title = title.replace("_", " - ").replace("(1)", "").strip(" -")
    return title if len(title) <= 52 else f"{title[:49]}..."


def music_menu(screen, fonts) -> None:
    """Let players choose and preview BGM from the main menu."""
    large, medium, small = fonts
    clock = pygame.time.Clock()
    ...
    choices = SOUND.music_choices()
    try:
        selected = choices.index(SOUND.music_name)
    except ValueError:
        selected = 0
    message = "UP/DOWN SELECT   ENTER PLAY   R OR F5 RESCAN   ESC BACK"
    ...
    while True:
        ...
        for event in game_events():
            ...
            if event.key == pygame.K_UP:
                ...
                playing = SOUND.select_music(choices[selected])
                message = f"NOW PLAYING: {_music_display_name(playing)}"
            elif event.key in (pygame.K_r, pygame.K_F5):
                playing = SOUND.reload_music()
                choices = SOUND.music_choices()
                try:
                    selected = choices.index(playing)
                except ValueError:
                    selected = 0
                message = "MUSIC FOLDER RESCANNED"
            elif handle_audio_hotkey(event):
                continue

        screen.fill(BG)
        draw_grid(screen)
        draw_text(screen, large, "CHOOSE BGM", (WIDTH // 2, 58), GREEN, center=True)
        draw_text(
            screen,
            small,
            "Choose Built-in Synth or any MP3, OGG, or WAV in the music folder.",
            (WIDTH // 2, 96),
            MUTED,
            center=True,
        )
        ...
        visible_count = 7
        start = max(0, min(selected - visible_count // 2, len(choices) - visible_count))
        end = min(len(choices), start + visible_count)
        y = 145
        for index in range(start, end):
            choice = choices[index]
            is_selected = index == selected
            is_playing = choice == SOUND.music_name
            color = GREEN if is_selected else WHITE
            pygame.draw.rect(screen, DARK_PANEL, (175, y - 22, 650, 44), border_radius=9)
            marker = ">" if is_selected else " "
            suffix = "  [PLAYING]" if is_playing else ""
            draw_text(
                screen,
                medium,
                f"{marker} {_music_display_name(choice)}{suffix}",
                (WIDTH // 2, y),
                color,
                center=True,
            )
        ...
        draw_text(screen, small, message, (WIDTH // 2, 555), MUTED, center=True)
        present(screen)
...
def network_test_menu(screen, fonts) -> Optional[NetworkTestPreset]:
    ...
    while True:
        ...
        draw_text(
            screen,
            small,
            "P1 = YOU • P2 = REMOTE CPU • IN-MEMORY LINK + ARTIFICIAL INTERNET DELAY",
            (WIDTH // 2, 91),
            MUTED,
            center=True,
        )
...
def _wait_for_loopback_test_pair(screen, fonts, preset: NetworkTestPreset):
    """Create the in-memory test pair while keeping the Pygame window responsive."""
    _, medium, small = fonts
    job = LoopbackPairJob()
    job.start()
    clock = pygame.time.Clock()
    ...
    while not job.done.is_set():
        ...
        screen.fill(BG)
        draw_grid(screen)
        draw_text(screen, medium, "BUILDING v30 NETWORK SIMULATOR", (WIDTH // 2, 235), GREEN, center=True)
        ...
        draw_text(screen, small, "Creating two in-memory endpoints on this PC...", (WIDTH // 2, 362), MUTED, center=True)
        present(screen)
...
def run_network_test(screen, fonts, preset: NetworkTestPreset) -> None:
    ...
    remote_cpu = DeterministicTestCpu()
    ...
    try:
        while running:
            ...
            while accumulator >= FIXED_DT:
                ...
                # the v30 in-memory test link. Only transport timing is artificial.
...
def online_lobby(screen, fonts) -> Optional[tuple[str, SecurePeer, int]]:
    ...
    code_value = ""
    status_message = "BOTH PLAYERS OPEN THIS LOBBY — WAIT TO HOST OR ENTER TO JOIN"
    status_color = MUTED
    ...
    # Fast rematch path: when a known online opponent is visible, preselect
    # them automatically. The lobby itself is already listening as a host, so
    # the player who wants to HOST simply waits; the player who wants to JOIN
    # presses ENTER once.
    if selected_index is None:
        for i, peer_info in enumerate(peers):
            if peer_info.online and trusted_code_for_peer(peer_info, port):
                selected_index = i
                code_value = trusted_code_for_peer(peer_info, port)
                status_message = f"{peer_info.name} READY — WAIT TO HOST OR ENTER TO JOIN"
                status_color = GREEN
                break
    ...
    while True:
        ...
        for event in game_events():
            ...
            if selected_index is not None and selected_index < len(peers):
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    # Each new lobby creates a fresh temporary TLS cert, so a
                    # previously trusted fingerprint can be stale. Always ask
                    # the visible peer for its CURRENT pairing code first.
                    status_message = f"REFRESHING SECURE INVITE FROM {selected.name}..."
                    status_color = MUTED
                    present(screen)
                    fetched = pairing_discovery.request_code(selected.address)
                    if fetched:
                    ...
                    if len(normalize_security_code(fingerprint)) < 16:
                        status_message = (
                            f"NO CURRENT PAIRING CODE FROM {selected.name} — "
                            "WAIT A SECOND AND PRESS ENTER AGAIN"
                        )
                        status_color = ERROR
                        continue
        ...
        draw_text(
            screen,
            small,
            "WAIT = HOST   ENTER = JOIN SELECTED/LAST OPPONENT   1-9 CHANGE PEER   ESC BACK",
            (WIDTH // 2, 574),
            MUTED,
            center=True,
        )
...
def run_match(screen, fonts, mode: str, peer: Optional[SecurePeer], local_player: int) -> bool:
    clock = pygame.time.Clock()
    accumulator = 0.0
    running = True
    ...
    return_to_online_lobby = False

    while running:
        dt = min(clock.tick(FPS) / 1000.0, 0.05)
        for event in game_events():
            if event.type == pygame.QUIT:
                session.request_session_exit()
            elif event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_ESCAPE, pygame.K_BACKSPACE):
                    session.request_session_exit()
                elif handle_audio_hotkey(event):
                    ...
                    # Rematch: same live connection, same host/guest roles.
                    session.request_restart()
                    render_state = RenderState.from_world(session.state)
                    accumulator = 0.0
                elif event.key == pygame.K_n and session.state.winner:
                    # New match: both return to quick lobby so roles can swap.
                    session.request_match_exit()
        ...
        if session.should_leave_online_session:
            # ESC / window close means online play is over for both peers.
            return_to_online_lobby = False
            running = False
            continue

        if session.should_leave_completed_match:
            # N means keep playing together, but return to quick lobby so host
            # and guest roles can be swapped.
            return_to_online_lobby = True
            running = False
            continue
        ...
        if session.state.winner:
            draw_text(
                screen, small,
                "R REMATCH    N NEW MATCH / SWAP HOST    ESC DISCONNECT",
                (WIDTH // 2, 112), GREEN, center=True
            )
        present(screen)
