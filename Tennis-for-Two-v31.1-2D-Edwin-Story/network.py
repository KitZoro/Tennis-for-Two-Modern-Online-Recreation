...
class SecurePeer:
    ...
    def __init__(self, sock: ssl.SSLSocket):
        ...
        # v29/v30: larger OS socket buffers for the 120 Hz transport.
        try:
    ...
    def _send_bytes(self, packet: bytes) -> None:
        """Send an entire byte string without resending an already-written prefix."""
        view = memoryview(packet)
        offset = 0
        stall_started = None

        while offset < len(view) and self.alive:
            try:
    ...
    def _writer(self) -> None:
        """Drain the outbox in small batches and send with partial-write safety."""
        try:
            while self.alive:
                try:
                    first = self.outbox.get(timeout=0.25)
                except queue.Empty:
                    continue
                ...
                try:
                    ...
                    if framed:
                        self._send_bytes(b"".join(framed))
                        self.packets_out += valid_count
                finally:
                    for _ in batch:
