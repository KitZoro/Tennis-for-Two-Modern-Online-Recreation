...
# v31.5 NETWORK STABILITY: single-thread TLS I/O
class SecurePeer:
    ...
    The on-wire format is unchanged: this is still TCP + TLS, protocol 31.
    """
    ...
    def __init__(self, sock: ssl.SSLSocket):
        ...
        try:
            ...
            # Windows exposes keepalive timing through SIO_KEEPALIVE_VALS.
            if hasattr(socket, "SIO_KEEPALIVE_VALS") and hasattr(self.sock, "ioctl"):
                self.sock.ioctl(socket.SIO_KEEPALIVE_VALS, (1, 15000, 5000))
    ...
    def _io_loop(self) -> None:
        try:
            while self.alive:
                # Transport heartbeat is independent from the Pygame loop.  If the
                # render thread stalls or the window is dragged, the network thread
                # still emits traffic and keeps the TLS/TCP path warm.
                now = time.monotonic()
                if now - self.last_sent >= 1.0 and self.outbox.empty() and not self._send_buffer:
                    try:
                        self.outbox.put_nowait({"type": "transport_keepalive"})
                    except queue.Full:
                        pass
    ...
    def close(self, reason: str = "Local match closed") -> None:
        self._terminate(reason or "Local match closed", "local_close")
