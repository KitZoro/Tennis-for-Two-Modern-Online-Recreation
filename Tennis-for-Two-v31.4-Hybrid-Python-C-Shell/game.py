...
from config import *
from native_core import deterministic_shot_error as native_deterministic_shot_error
...
def deterministic_shot_error(state: WorldState, side: int, power: int) -> float:
    # Native helper is optional and startup-self-tested.  The Python fallback
    # implements the exact same canonical formula, so gameplay remains usable
    # even on machines without a compiler or native library.
    return native_deterministic_shot_error(
        state.frame, side, state.score1, state.score2, power
    )
